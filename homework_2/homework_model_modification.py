...
from homework_2.utils import make_regression_data, mse, log_epoch, RegressionDataset

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

class LinearRegression(nn.Module): # наследуем от nn.Module для автоматического вычисления градиентов
    """ 
    Модель линейной регрессии с L1 и L2 регуляризацией.
    Эта модель реализует линейную регрессию с возможностью добавления L1 и L2 регуляризации.
    """
...
def compute_metrics(y_true, y_pred, y_prob, num_classes):
    ...
    """
    precision = precision_score(y_true, y_pred, average='weighted') # Параметр average='weighted': усреднение метрик по классам с учётом их весов
